    ...
    * a Flatten layer
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(
             # implement model here with layers from torch.nn
    ...
    def forward(self, image):
        # revise to call model on the given image
...
class ClassifyNet(nn.Module):
    ...
    * a linear layer with input side hidden_layer_size and output size output_size
    ...
    def forward(self, image_features):
        # revise to call model on the given image_features
...
class ImageClassifyModel(object):
    ...
    def __init__(self, image_embed_net,
      ...
      self.exclude_embed_params = exclude_embed_params
      self.parameters = []
        
      if exclude_embed_params:
        self.parameters = self.image_classify_net.parameters()
      else:
        self.parameters = [self.image_embed_net.parameters(), self.image_classify_net.parameters()]
    ...
    def classify(self, image):
      # revise to return output of image_classify_net
      out = self.image_embed_net(image)
      return self.image_classify_net(out)
