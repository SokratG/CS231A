...
def compute_camera_matrix(real_XY, front_image, back_image):
    ...
    h, w = front_image.shape

    p_front = np.hstack([front_image, np.ones((h, 1))])
    p_back = np.hstack([back_image, np.ones((h, 1))])
    p = np.vstack([p_front, p_back]) # build image points

    P_front_real_z = np.hstack([real_XY, np.zeros((h, 1)), np.ones((h, 1))])
    P_back_real_z = np.hstack([real_XY, np.full((h, 1), 150.0), np.ones((h, 1))])
    P = np.vstack([P_front_real_z, P_back_real_z]) # build scene world points

    # calc least square problem
    M = np.linalg.lstsq(P, p, rcond=None)[0].transpose()
    # last column for affine transformation(ortho projection)
    M[-1] = np.array([0.0, 0.0, 0.0, 1.0])
...
def rms_error(camera_matrix, real_XY, front_image, back_image):
    ...
    P_front_real_z = np.hstack([real_XY, np.zeros((h, 1)), np.ones((h, 1))])
    P_back_real_z = np.hstack([real_XY, np.full((h, 1), 150.0), np.ones((h, 1))])
    P = np.vstack([P_front_real_z, P_back_real_z]) # build scene points

    p_prime = P.dot(camera_matrix.T) # back projection(reprojecting) scene points to image
    N = P.shape[0] # total number of points
    rms_error = np.sqrt(np.sum(np.power(p - p_prime, 2)) / N) # compute root mean square error
